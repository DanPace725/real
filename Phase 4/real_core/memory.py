...
class EpisodicMemory:
    ...
    def consolidate_three_tier(
        ...
        attractors = sorted(self.entries, key=lambda e: e.coherence, reverse=True)[:keep_attractors]
        surprises = sorted(self.entries, key=lambda e: abs(e.delta), reverse=True)[:keep_surprises]
        boundaries = sorted(self.entries, key=lambda e: abs(e.coherence - threshold))[:keep_boundaries]
